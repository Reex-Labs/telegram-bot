...
const transferScene = new Scenes.BaseScene<Scenes.SceneContext>(
  "transferScene"
);
...
transferScene.on("message", async (ctx: any) => {
  if (!amount && !to) {
    ...
    if (isValidAmount(number)) {
      ...
      ctx.replyWithMarkdown(
        "С цифрой что-то не так: либо это не цифра, либо она меньше 0.000001. Введите еще раз.",
        backKeyboard
      );
    }
    return;
  ...
  if (amount && !to) {
    ...
    if (isValidAddress(address)) {
      to = address;
      ctx.replyWithMarkdown(
        messages.CONFIRM_SEND(to, amount),
        inlineSendConfirm
      );
    } else {
